let doms_url = "doms.skbroadband.com";
...
let getPlusDate = function (intDay, hh, mi, ss) {
    ...
    plusDate = now_day.toISOString().substr(0, 10) + " " + hh + ":" + mi + ":" + ss;
        ...
        if (queryStr[1].includes('&')) {
            ...
            for (var i = 0; i < queryWord.length; i++) {
                ...
                try {
                    ...
                    if (queryWords[1] != "") result = result + " " + decodeURIComponent(queryWords[1]);
                } catch (e) {
                    continue;
                }
            }
...
let cookie_id = doms_year + "" + doms_month + "" + doms_day + "" + doms_hour + "" + doms_min + "" + doms_sec + "" + generateRandomString(5);
let cookie_cnt = 1;
let cookie_create_date = doms_year + "-" + doms_month + "-" + doms_day + " " + doms_hour + ":" + doms_min + ":" + doms_sec;
let cookie_last_date = doms_year + "-" + doms_month + "-" + doms_day + " " + doms_hour + ":" + doms_min + ":" + doms_sec;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if ((doms_referer != "") && (doms_referer.includes(doms_domain) == false)) {
        ...
        con_keyword = getKeywordFromUrl(doms_referer) + getKeywordFromUrl(window.location.href);
    ...
    const doms_condition = {
        "DOMS_KEY": doms_securekey,
        "CAM_TARGET": doms_cam_target,
        "THIS_URL": window.location.href,
        "DEVICE_TYPE": doms_device_type,
        "TG1_SUB": doms_tg1_sub,
        "REFER_URL": con_doms_referer,
        "KEYWORD": (con_keyword),
        "EXCEPT_CA_NO": dup_cookie_except_ca_no.join()
    };
        ...
        for (var i = 0; i < data.length; i++) {
            if (cookie_last_date < data[i][3]) {
                if (caNo == data[i][0]) {
                    // 동일 번호가 있다면
                    isExsist = true;
                    arrLimitList.push([data[i][0], parseInt(data[i][1]), parseInt(data[i][2]) + 1, expiredDate]);
                } else {
                    arrLimitList.push([data[i][0], parseInt(data[i][1]), data[i][2], data[i][3]]);
                }
            }
        }

        if (isExsist == false) arrLimitList.push([caNo, parseInt(limitCnt), displayCnt, expiredDate]);
...
const addPurposeList = (caNo, caPurpose, caPurposeUrl, msgType) => {
    ...
    if ((sessionStorage.getItem("s_purpose_list") == null) || (sessionStorage.getItem("s_purpose_list") == "")) {
        ...
        if (isExsist == false) arrPurposeList.push([caNo, caPurpose, caPurposeUrl, msgType]);
...
const settingCookie = () => {
    ...
    if (!localStorage.getItem('DOMS_COOKIE')) {
        ...
        if ((sessionStorage.getItem("s_cookie_cnt") == null) || (sessionStorage.getItem("s_cookie_cnt") == "")) {
        ...
        cookie_last_date = doms_year + "-" + doms_month + "-" + doms_day + " " + doms_hour + ":" + doms_min + ":" + doms_sec;
...
const settingVisitCnt = () => {
    ...
    if ((sessionStorage.getItem("s_visit_cnt") == null) || (sessionStorage.getItem("s_visit_cnt") == "")) {
        sessionStorage.setItem("s_visit_cnt", 1);
    } else {
        sessionStorage.setItem("s_visit_cnt", parseInt(sessionStorage.getItem("s_visit_cnt")) + 1);
        if (parseInt(sessionStorage.getItem("s_visit_cnt")) > 3) doms_cam_target = doms_cam_target + ",SEARCH";
    }
...
const settingDomainCnt = () => {
    ...
    if ((localStorage.getItem('DOMS_DOMAIN') == null) || (localStorage.getItem('DOMS_DOMAIN') == "")) {
...
const previewDomsPopup = (doms_ca_no) => {
    ...
    request.done(function (result) {
        ...
        if (resultData.CA_MSG_TYPE == "6") {
            ...
            if ((window.location.href.includes('bdirectshop.com')) && (doms_is_mobile == true)) {
    ...
    request.done(function (result) {
        ...
        if ((resultData == "undefined") || (resultData == null) || (resultData == "")) {
            ...
            if (resultData[0].CA_VIEW_POINT == "AT_ONCE") {
                ...
                saveDomsLog("SHOW", resultData[0].CA_NO, resultData[0].CA_MSG_TYPE, resultData[0].EXPOSURE_LIMIT_CNT, resultData[0].CA_PURPOSE, resultData[0].CA_PURPOSE_URL);
                    ...
                    window.addEventListener('popstate', function (event) {
                        ...
                        setTimeout(() => {
                            ...
                            if ((doms_open == true) && (is_head == false)) {
                                ...
                                if (is_head == false) history.pushState(null, null, window.location.pathname);
                            }
...
let activateLink = function () {
    ...
    linkClickClass.forEach((target) => target.addEventListener("click", function () {

        saveDomsLog(this.getAttribute('data-action'), this.getAttribute('data-caNo'), this.getAttribute('data-msgType'), this.getAttribute('data-limitCnt'), "", "");

        if ((this.getAttribute('data-action') == "CONTACT") || (this.getAttribute('data-action') == "LINKCLICK") || (this.getAttribute('data-action') == "SIGNUP")) {
            ...
            domsPopupClose(this.getAttribute('data-msgType'), this.getAttribute('data-action'));
        }
...
const saveDomsLog = (action, doms_ca_no, msg_type, limit_cnt, ca_purpose, ca_purpose_url, multi_no = 0) => {
    if (msg_type === "8") {
        if (action !== "SHOW" && action !== "CLOSECLICK" && action !== "VISIT") {
            multi_no = document.querySelector('.slide-item.swiper-slide.swiper-slide-active').getAttribute('data-multi-no') ?? 0;
        }
    }
    ...
    var request = $.ajax({
        ...
        data: {
            ...
            EVENT_TYPE: action,
            MSG_TYPE: msg_type,
            CA_REFERER: doms_referer,
            COOKIE_CNT: cookie_cnt,
            EXP_TIME: doms_exp_time,
            CA_SESSION_ID: session_id,
            CA_COOKIE_ID: cookie_id,
            CA_FULL_URL: window.location.href,
            CA_TITLE: document.title,
            CA_IP: doms_remote_ip,
            CA_MENU_NO: skb_ca_menu_no,
            CA_C_CODE: skb_ca_c_code,
            MULTI_NO: multi_no
        },
        dataType: "json"
    });
    ...
    request.done(function (result) {
        ...
        //console.log("limit_cnt : " +limit_cnt);
        //console.log("action : " +action);
        //console.log("ca_purpose : " +ca_purpose);
        ...
        if ((ca_purpose == "ONLINE") && (action == "SHOW")) {
            if (device_type == "P") ca_purpose_url = location.origin + "/shop/com/cs/done.do";
            if (device_type == "M") ca_purpose_url = location.origin + "/shop/com/cs/done.do";
...
let makePopupHtml = function (data, preview, multiList) {
    ...
    if ((dataBuiDevice == "laptop") && (data.MSG_BUTTON_URL_TARGET_P == "N")) landPageUrlTarget = `target="_blank"`;
    if ((dataBuiDevice == "mobile") && (data.MSG_BUTTON_URL_TARGET_M == "N")) landPageUrlTarget = `target="_blank"`;
    ...
    if (data.CA_MSG_TYPE == "1") {
        html = `<div class="campaign-popup module-b style-a type-a small" 
                     style="${addPdding}"
                     id="campaignDefaultPopupTypeA" 
                     data-campaign-toggle="campaignDefaultPopup" 
                     data-campaign-dimmed="` + dataBuiDimmed + `" 
                     data-campaign-device="` + dataBuiDevice + `">`;
    } else if (data.CA_MSG_TYPE == "2") {
        html = `<div class="campaign-popup module-b style-a type-a medium" id="campaignDefaultPopupTypeB" data-campaign-toggle="campaignDefaultPopup" data-campaign-dimmed="` + dataBuiDimmed + `" data-campaign-device="` + dataBuiDevice + `">`;
    } else if (data.CA_MSG_TYPE == "3") {
        html = `<div class="campaign-popup module-b style-b type-a medium" 
                    style="${addPdding}"
        ...
        if (data.MSG_IMG_TF == "Y") {
            html = `<div class="campaign-popup module-b style-b type-c medium" 
                        id="campaignScrollPopupTypeB" 
                        data-campaign-toggle="campaignScrollPopup" 
                        data-campaign-dimmed="` + dataBuiDimmed + `" 
                        data-campaign-device="` + dataBuiDevice + `" 
                        style="--campaign-profile-image: url(//${doms_url}/images/campaign/` + data.MSG_IMG_NM01 + `); ${addPdding}">`;
        } else {
            html = `<div class="campaign-popup module-b style-b type-c medium" 
                        style="${addPdding}"
                        id="campaignScrollPopupTypeB" 
        ...
        html = `<div style="--effect: ${efftect};" class="campaign-popup module-c style-a type-a medium" id="campaignFloatingPopupTypeA" data-campaign-toggle="campaignFloatingPopup" data-campaign-dimmed="` + dataBuiDimmed + `" data-campaign-device="` + dataBuiDevice + `">`;
    } else if (data.CA_MSG_TYPE == "6") {

        html = `<div class="campaign-popup module-a style-a type-a medium" id="campaignLeadBoardTypeA" data-campaign-toggle="campaignLeadBoard" data-campaign-device="` + dataBuiDevice + `" style="--campaign-background-color:` + data.MSG_BAN_BG_COLOR + `;--campaign-unit-color:` + data.MSG_BAN_TEXT_COLOR + `;">`;
    } else if (data.CA_MSG_TYPE == "7") {
        html =
            `<div style="--effect: ${efftect}; --campaign-background-color:${ data.MSG_BAN_BG_COLOR }" class="campaign-popup module-d style-a type-a medium campaign-toggle-target" id="campaignActionFloatingPopupTypeA" data-campaign-toggle="campaignActionFloatingPopup" data-campaign-dimmed="` + dataBuiDimmed + `" data-campaign-device="` + dataBuiDevice + `">`;
    } else if (data.CA_MSG_TYPE == "8") {
        html = `<div style="--effect: ${efftect}; ${addPdding}" 
    ...
    if (data.CA_MSG_TYPE == "8") {
        ...
        for(let i=0; i<multiList.length; i++) {
            ...
            if ((dataBuiDevice == "laptop") && (multiData.msgButtonUrlTargetP == "N")) landPageUrlTarget = `target="_blank"`;
            if ((dataBuiDevice == "mobile") && (multiData.msgButtonUrlTargetM == "N")) landPageUrlTarget = `target="_blank"`;
            ...
            let strGa4 = `data-ga4id="` + skb_data_ga4id_scroll + `" data-ga4="ga4" data-ga4type="more" data-ga4location="all" data-ga4desc="` + skb_data_ga4desc_scroll + `" `;

            let landingBtnHtml = '';
            if (multiData.msgButtonTf == "Y") {
                landingBtnHtml += ` <a ${landPageUrlTarget} class="btn type-cta attr-move doms_click" style="background-color:` + multiData.msgButtonBgColor + `;border-color:` + multiData.msgButtonBgColor + `" href="` + landPageUrl + `" id="BDS_DOMS_action" data-action="LINKCLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" ` + strGa4 + `><span class="btn-text" style="pointer-events: none; color: ${multiData.msgButtonColor};">` + multiData.msgButton + `</span></a>`;
            }

            let leaveBtnHtml = ''
            if (data.MSG_LEAVE_TF == "Y") {
                if ((window.location.href.includes('btvcable.shop')) && (dataBuiDevice == "mobile")) {
                    leaveBtnHtml += ` <a class="btn type-cta attr-send doms_click" style="border-color:` + data.MSG_LEAVE_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_LEAVE_COLOR + `" href="#consultationRequest" id="BDS_DOMS_contact" data-action="CONTACT" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onClick="consultNewRequestPopup.active('consultNewRequest'); return false;"><span class="btn-text" style="color:` + data.MSG_LEAVE_COLOR + `">가입상담 신청</span></a>`;
                } else if ((window.location.href.includes('bworld.co.kr')) && (dataBuiDevice == "mobile")) {
                    leaveBtnHtml += ` <a class="btn type-cta attr-send doms_click" style="border-color:` + data.MSG_LEAVE_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_LEAVE_COLOR + `" href="#consultationRequest" id="BDS_DOMS_contact" data-action="CONTACT" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onclick="contentPopup.active('consultationRequest', getTelephoneNumber); return false;"><span class="btn-text" style="color:` + data.MSG_LEAVE_COLOR + `">가입상담 신청</span></a>`;
                } else {
                    leaveBtnHtml += ` <a class="btn type-cta attr-send doms_click" style="border-color:` + data.MSG_LEAVE_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_LEAVE_COLOR + `" href="#consultationRequest" id="BDS_DOMS_contact" data-action="CONTACT" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onclick="contentPopup.active('consultationRequest'); return false;"><span class="btn-text" style="color:` + data.MSG_LEAVE_COLOR + `">가입상담 신청</span></a>`;
                }
            }

            let registBtnHtml = '';
            if (data.MSG_REGIST_TF == "Y") {
                registBtnHtml += ` <a class="btn type-cta attr-join doms_click" style="border-color:` + data.MSG_REGIST_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_REGIST_COLOR + `" href="` + skb_baro_url + `" data-action="SIGNUP" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" target="_blank" id="BDS_DOMS_baro"><span class="btn-text" style="color:` + data.MSG_REGIST_COLOR + `">바로가입</span></a>`;
            }

            let callBtnHtml = '';
            if ((data.MSG_MOBILE_CALLING_TF == "Y") && (doms_is_mobile == true) && (skb_chkMoblieCall !== "time_off")) {
                callBtnHtml += ` <a class="btn type-cta attr-call doms_click" style="border-color:` + data.MSG_MOBILE_CALLING_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_MOBILE_CALLING_COLOR + `" href="tel:` + skb_tel_num + `" id="BDS_DOMS_phone_call" data-action="PHONECLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `"><span class="btn-text" style="color:` + data.MSG_MOBILE_CALLING_COLOR + `">가입상담 전화</span></a>`;
            }
            ...
            if (landingBtnHtml === '' && leaveBtnHtml === '' && registBtnHtml === '' && callBtnHtml === '') {
                boardInformTemplate = '';
            }
        ...
        // 이미지 사용 1,2,3,5,6,7
        if ((data.CA_MSG_TYPE == "1") || (data.CA_MSG_TYPE == "2") || (data.CA_MSG_TYPE == "3") || (data.CA_MSG_TYPE == "5") || (data.CA_MSG_TYPE == "6") || (data.CA_MSG_TYPE == "7")) {
            ...
            if (data.CA_MSG_TYPE == "6" || data.CA_MSG_TYPE == "7") {
                ...
                html = html + `<span class="board-cover doms_click" href="` + landPageUrl + `" id="BDS_DOMS_action" data-action="LINKCLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" ><img src="//${doms_url}/images/campaign/` + data.MSG_IMG_NM01 + `" alt="` + data.MSG_IMG_ALTER01 + `" /></span>`;

            } else {

                if (data.MSG_IMG_NM01 != "") html = html + `<span class="board-cover"><img src="//${doms_url}/images/campaign/` + data.MSG_IMG_NM01 + `" alt="` + data.MSG_IMG_ALTER01 + `" /></span>`;
        ...
        // 1, 2, 4, 5, 7
        if ((data.CA_MSG_TYPE == "1") || (data.CA_MSG_TYPE == "2") || (data.CA_MSG_TYPE == "4") || (data.CA_MSG_TYPE == "5") || (data.CA_MSG_TYPE == "7")) {
            ...
            if (data.MSG_TITLE_COLOR !== "") msgTitleColor = `style="color:` + data.MSG_TITLE_COLOR + `"`;
            if (data.MSG_CONTENTS_COLOR !== "") msgContentsColor = `style="color:` + data.MSG_CONTENTS_COLOR + `"`;

            if (data.MSG_TITLE_TF == "Y") html = html + ` <p class="board-subject"><span class="board-name" ` + msgTitleColor + `>` + data.MSG_TITLE + `</span></p>`;

            if (data.MSG_CONTENTS_TF == "Y" && data.MSG_CONTENTS != '') html = html + ` <p class="board-summary" ` + msgContentsColor + `>` + data.MSG_CONTENTS.replace(/(\r|\r\n)/g, "<br />") + `</p>`;
        ...
        html = html + ` <div class="board-util">
                            <span class="button-area" ` + msgButtonTf + `>`;

        let strGa4 = "";

        // 버튼 영역 1, 2, 4번
        if ((data.CA_MSG_TYPE == "1") || (data.CA_MSG_TYPE == "2") || (data.CA_MSG_TYPE == "4")) {

            if ((data.CA_MSG_TYPE == "1") || (data.CA_MSG_TYPE == "2")) {
                strGa4 = `data-ga4id="` + skb_data_ga4id_popup + `" data-ga4="ga4" data-ga4type="more" data-ga4location="all" data-ga4desc="` + skb_data_ga4desc_popup + `" `;
            } else {
                strGa4 = `data-ga4id="` + skb_data_ga4id_scroll + `" data-ga4="ga4" data-ga4type="more" data-ga4location="all" data-ga4desc="` + skb_data_ga4desc_scroll + `" `;
            }
            ...
            if (data.MSG_BUTTON_TF == "Y") {
                html = html + ` <a ${landPageUrlTarget} class="btn type-cta attr-move doms_click" style="background-color:` + data.MSG_BUTTON_BG_COLOR + `;border-color:` + data.MSG_BUTTON_BG_COLOR + `" href="` + landPageUrl + `" id="BDS_DOMS_action" data-action="LINKCLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" ` + strGa4 + `><span class="btn-text" style="pointer-events: none; color: ${ data.MSG_BUTTON_COLOR }">` + data.MSG_BUTTON + `</span></a>`;
            }

            if (data.MSG_LEAVE_TF == "Y") {
                if ((window.location.href.includes('btvcable.shop')) && (dataBuiDevice == "mobile")) {
                    html = html + ` <a class="btn type-cta attr-send doms_click" style="border-color:` + data.MSG_LEAVE_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_LEAVE_COLOR + `" href="#consultationRequest" id="BDS_DOMS_contact" data-action="CONTACT" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onClick="consultNewRequestPopup.active('consultNewRequest'); return false;"><span class="btn-text" style="color:` + data.MSG_LEAVE_COLOR + `">가입상담 신청</span></a>`;
                } else if ((window.location.href.includes('bworld.co.kr')) && (dataBuiDevice == "mobile")) {
                    html = html + ` <a class="btn type-cta attr-send doms_click" style="border-color:` + data.MSG_LEAVE_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_LEAVE_COLOR + `" href="#consultationRequest" id="BDS_DOMS_contact" data-action="CONTACT" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onclick="contentPopup.active('consultationRequest', getTelephoneNumber); return false;"><span class="btn-text" style="color:` + data.MSG_LEAVE_COLOR + `">가입상담 신청</span></a>`;
                } else {
                    html = html + ` <a class="btn type-cta attr-send doms_click" style="border-color:` + data.MSG_LEAVE_LINE_COLOR + `;--campaign-button-border-color:` + data.MSG_LEAVE_COLOR + `" href="#consultationRequest" id="BDS_DOMS_contact" data-action="CONTACT" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onclick="contentPopup.active('consultationRequest'); return false;"><span class="btn-text" style="color:` + data.MSG_LEAVE_COLOR + `">가입상담 신청</span></a>`;
        ...
        if (data.CA_MSG_TYPE == "3") {
            ...
            if (data.MSG_BUTTON_TF == "Y") {
                html = html + ` <a ${landPageUrlTarget} class="btn type-cta attr-move doms_click" style="background-color:` + data.MSG_BUTTON_BG_COLOR + `;border-color:` + data.MSG_BUTTON_BG_COLOR + `" href="` + landPageUrl + `" id="BDS_DOMS_action" data-action="LINKCLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" ` + strGa4 + `><span class="btn-text" style="pointer-events: none;">` + data.MSG_BUTTON + `</span></a>`;
            }
        ...
        // 5, 6번
        if ((data.CA_MSG_TYPE == "5") || (data.CA_MSG_TYPE == "6") || (data.CA_MSG_TYPE == "7")) {

            let landPageUrl = data.MSG_BUTTON_PC_URL;
            if (doms_is_mobile == true) landPageUrl = data.MSG_BUTTON_MO_URL;

            if (data.CA_MSG_TYPE == "5") {
                strGa4 = `data-ga4id="` + skb_data_ga4id_floating + `" data-ga4="ga4" data-ga4type="more" data-ga4location="all" data-ga4desc="` + skb_data_ga4desc_floating + `" `;
            } else {
                strGa4 = `data-ga4id="` + skb_data_ga4id_lead + `" data-ga4="ga4" data-ga4type="more" data-ga4location="all" data-ga4desc="` + skb_data_ga4desc_lead + `" `;
            }
            if (data.MSG_BUTTON_TF == "Y") {
                html = html + ` <a ${landPageUrlTarget} class="btn type-goto attr-move doms_click" style="background-color:` + data.MSG_BUTTON_BG_COLOR + `;border-color:` + data.MSG_BUTTON_BG_COLOR + `" href="` + landPageUrl + `" id="BDS_DOMS_action" data-action="LINKCLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" ` + strGa4 + `><span class="btn-text" style="pointer-events: none;">랜딩 페이지로 이동</span></a>`;
            }
        }
...
<div class="popup-local-util">`;
    ...
    if ((data.CA_MSG_TYPE == "1") || (data.CA_MSG_TYPE == "2") || (data.CA_MSG_TYPE == "3") || (data.CA_MSG_TYPE == "4") || (data.CA_MSG_TYPE == "6") || (data.CA_MSG_TYPE == "8")) {

        if (data.MSG_REVIEW_TF == "Y") {


            if ((data.MSG_REVIEW_DAYS == null) || (data.MSG_REVIEW_DAYS == "")) data.MSG_REVIEW_DAYS = 1;
...
<input class="form-elem" type="checkbox" data-caNo="` + data.CA_NO + `" data-msgReviewDay="` + data.MSG_REVIEW_DAYS + `" id="doms_msg_review_day"/>
<span class="form-text">` + str_msg_review + `</span>
</label>
</span>
</div>
</div>
    ...
    if (data.CA_MSG_TYPE == "7") {
        html += `<input style="display:none;" class="form-elem" type="checkbox" data-caNo="` + data.CA_NO + `" data-msgReviewDay="1" id="doms_msg_review_day" checked/>`;
    }

    html = html + `
<div class="button-display">
<span class="button-area">
<button class="btn popup-close doms_click" type="button" data-action="CLOSECLICK" data-caNo="` + data.CA_NO + `" data-msgType="` + data.CA_MSG_TYPE + `" data-limitCnt="` + data.EXPOSURE_LIMIT_CNT + `" onclick="domsPopupClose('` + data.CA_MSG_TYPE + `', 'CLOSECLICK')">
...
let domsPopupOpen = function (msgType) {
    ...
    if (msgType == "1") {
        ...
        // 모바일 인경우
        if ((window.location.href.includes('btvcable.shop')) && (doms_is_mobile == true)) {

            setTimeout(() => {

                linePopupHeight = parseInt(document.getElementById('campaignLeadBoardTypeA').offsetHeight);
                ...
                if ($(".top_banner_new").css("display") == "none") {
                    ...
                    $(".page-head ~ .page-body").css({ "margin-top": (linePopupHeight + linecontainerHeight - lineHeadHeight) + "px" });
                    $("#container").css({ "padding-top": (linePopupHeight + linecontainerHeight - lineHeadHeight) + "px" });

                } else {

                    $(".top_banner_new").css({ "top": linePopupHeight + "px" });
                    $(".page-head").css({ "position": "fixed", "top": (linePopupHeight + lineHeadHeight) + "px" });
                    $(".page-head ~ .page-body").css({ "margin-top": (linePopupHeight + linecontainerHeight) + "px" });
                    $("#container").css({ "padding-top": (linePopupHeight + linecontainerHeight) + "px" });
        ...
        if ((window.location.href.includes('bdirectshop.com')) && (doms_is_mobile == false)) {
        ...
        if ((window.location.href.includes('btvcable.shop')) && (doms_is_mobile == false)) {
...
let domsPopupClose = function (msgType, action) {
    ...
    if (msgType == "1") {
        ...
        if ((window.location.href.includes('btvcable.shop')) && (doms_is_mobile == true)) {
            ...
            if ($(".top_banner_new").css("display") == "none") {
                ...
                $(".page-head ~ .page-body").css({ "margin-top": (linecontainerHeight - lineHeadHeight) + "px" });
                $("#container").css({ "padding-top": (linecontainerHeight - lineHeadHeight) + "px" });

            } else {

                $(".top_banner_new").css({ "top": "0px" });
                $(".page-head").css({ "position": "fixed", "top": (lineHeadHeight) + "px" });
                $(".page-head ~ .page-body").css({ "margin-top": (linecontainerHeight) + "px" });
    ...
    if (action == "CLOSECLICK") {
        ...
        if ((document.getElementById("doms_msg_review_day") != null) && (document.getElementById("doms_msg_review_day") != "undefined")) {

            if (document.getElementById("doms_msg_review_day").checked == true) {

                let expiredDate = getPlusDate(parseInt(document.getElementById("doms_msg_review_day").getAttribute('data-msgReviewDay')), doms_hour, doms_min, doms_sec);

                if ((localStorage.getItem('DOMS_REVIEW_LIST') == null) || (localStorage.getItem('DOMS_REVIEW_LIST') == "")) {
                    arrReviewList.push([document.getElementById("doms_msg_review_day").getAttribute('data-caNo'), parseInt(document.getElementById("doms_msg_review_day").getAttribute('data-msgReviewDay')), expiredDate]);
                } else {
                    // review list 재설정
                    arrReviewList.length = 0;
                    const data = JSON.parse(localStorage.getItem('DOMS_REVIEW_LIST'));
...
$(document).on("click", ".btn_to_banner_new", function () {
    ...
    if (document.getElementById('campaignLeadBoardTypeA').classList.contains("active") == true) {
    ...
    if ((window.location.href.includes('btvcable.shop')) && (doms_is_mobile == true)) {
        ...
        setTimeout(() => {
            ...
            if (document.getElementById('campaignLeadBoardTypeA') != null) {
                ...
                if ((document.getElementById('campaignLeadBoardTypeA').classList.contains("active") == true) && ($(".top_banner_new").css("display") == "block")) {
                ...
                } else if ((document.getElementById('campaignLeadBoardTypeA').classList.contains("active") == true) && ($(".top_banner_new").css("display") == "none")) {
                ...
                } else if ((document.getElementById('campaignLeadBoardTypeA').classList.contains("active") == false) && ($(".top_banner_new").css("display") == "block")) {
...
/*
fetch("http://doms.skbroadband.com/api/v1/preview", {
method: "POST",
mode: 'no-cors',
body: JSON.stringify({
DOMS_KEY: "P3QerWZfzEZoK66PesI072xbuQGYoKWi",
CA_NO: "31"
}),
})
...
console.log(cookie_id);
console.log(cookie_cnt);
console.log(cookie_create_date);
console.log(cookie_last_date);
console.log(cookie_last_url);
console.log(cookie_is_counsel);
*/
...
const initSwiper = function() {
    $('#campaignScrollPopupTypeC .scroll-slides').append(`<div class="swiper-pagination"></div>`);
