		...
		publicAPIs.update = function() {			
			...
			Array.prototype.forEach.call(navItems, function(navItem) {
				...
				if (subList != null) {
					...
					const navName = navItem.querySelector(settings.navName);
					const expandButton = document.createElement(settings.button);			
					expandButton.type = 'button';
					expandButton.className = settings.buttonClass;
					
					// navItem.insertBefore(expandButton, navName); // button을 naviName 앞에 추가
					navName.appendChild(expandButton); // button을 naviName 뒤에 추가
...
(function (root, factory) {
	...
	var Constructor = function(selector, options) {
		...
		publicAPIs.update = function() {
			...
			Array.prototype.forEach.call(formElems, function(value, index, array) {
				publicAPIs.myForm[index] = {
					...
					formElem: value,
					formFunc: value.parentElement.querySelector('.form-func'),
			...
			for (let i = 0; i < formIndex; i++) {
				...
				buiFormCheckTyped(publicAPIs.myForm[i].formElem);
				if (publicAPIs.myForm[i].formFunc != null) publicAPIs.myForm[i].formElem.style.setProperty('padding-right', publicAPIs.myForm[i].formFunc.offsetWidth + 'rem');
				
				// buiFormCancel
				if (publicAPIs.myForm[i].formElem.hasAttribute('data-bui-form-cancel')) {

					let buiFormCancel = publicAPIs.myForm[i].form.querySelector('.form-cancel');

					if (!buiFormCancel) {
						buiFormCancel = document.createElement('span');
						buiFormCancel.className = "form-cancel";
						...
						if (publicAPIs.myForm[i].formFunc != null) {
							...
							// mouse event
							publicAPIs.myForm[i].formElem.addEventListener("mousemove", function(event) {
								buiFormCancelAction(event, publicAPIs.myForm[i].formElem, publicAPIs.myForm[i].formFunc, buiFormCancel);
							});

							publicAPIs.myForm[i].formElem.addEventListener("mousedown", function(event) {
								var xStart = publicAPIs.myForm[i].formFunc.offsetLeft + buiFormCancel.offsetLeft;
								var yStart = publicAPIs.myForm[i].formFunc.offsetTop + buiFormCancel.offsetTop;
							...
							// mouse event
							publicAPIs.myForm[i].formElem.addEventListener("mousemove", function(event) {
								buiFormCancelAction(event, publicAPIs.myForm[i].formElem, publicAPIs.myForm[i].formUtil, buiFormCancel);
							});

							publicAPIs.myForm[i].formElem.addEventListener("mousedown", function(event) {
								var xStart = publicAPIs.myForm[i].formUtil.offsetLeft + buiFormCancel.offsetLeft;
								var yStart = publicAPIs.myForm[i].formUtil.offsetTop + buiFormCancel.offsetTop;
								var xEnd = xStart + buiFormCancel.offsetWidth;
								var yEnd = yStart + buiFormCancel.offsetHeight;
				...
				publicAPIs.myForm[i].formElem.addEventListener("input", function() {
					if (publicAPIs.myForm[i].formElem.value.length > 0) {
						publicAPIs.myForm[i].form.classList.add('typed');
						if (publicAPIs.myForm[i].formUtil != null) publicAPIs.myForm[i].formElem.style.setProperty('padding-right', publicAPIs.myForm[i].formUtil.offsetWidth + 'rem');
						if (publicAPIs.myForm[i].formFunc != null) publicAPIs.myForm[i].formElem.style.setProperty('padding-right', publicAPIs.myForm[i].formFunc.offsetWidth + 'rem');
					} else {
						publicAPIs.myForm[i].form.classList.remove('typed');
						if (publicAPIs.myForm[i].formUtil != null) publicAPIs.myForm[i].formElem.style.removeProperty('padding-right');
						if (publicAPIs.myForm[i].formFunc != null) publicAPIs.myForm[i].formElem.style.setProperty('padding-right', publicAPIs.myForm[i].formFunc.offsetWidth + 'rem');
					}
				});

				publicAPIs.myForm[i].formElem.addEventListener("mouseover", function() {
					if (publicAPIs.myForm[i].formUtil != null) publicAPIs.myForm[i].formElem.style.setProperty('padding-right', publicAPIs.myForm[i].formUtil.offsetWidth + 'rem');
					if (publicAPIs.myForm[i].formFunc != null) publicAPIs.myForm[i].formElem.style.setProperty('padding-right', publicAPIs.myForm[i].formFunc.offsetWidth + 'rem');
				});

				publicAPIs.myForm[i].formElem.addEventListener("mouseout", function() {
				...
				publicAPIs.myForm[i].formElem.addEventListener("focusin", function() {
					...
					console.log(publicAPIs.myForm[i].formElem.matches(':hover'));
				});
		...
		// buiFormCheckTyped
		function buiFormCheckTyped(formElem) {
