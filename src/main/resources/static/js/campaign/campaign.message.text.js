const campaignMessageText = (() => {
    ...
    const chkMsgImgTf = document.querySelector('#msgImgTf');                    // 이미지 버튼 사용 여부
    ...
    const viewImgs = document.querySelectorAll('.campaign-popup');                  // 이미지 선택시 미리보기
    ...
    return {
        init: function() {
            ...
            chkMsgImgTf.addEventListener('click', this.setDisplay);
            ...
            if(e.target.files && e.target.files[0]) {
                ...
                reader.onload = function(e) {
                    viewImgs.forEach(element => {
                        element.setAttribute('style', '--campaign-profile-image: url(' + e.target.result + ')');
                    });
                };
        ...
        setMsgContents: function(e) {
            ...
            const view = element.querySelector('.board-summary');
                if(elem.id == 'msgContents') {
                    campaignMessageText.setContentsRow(e);
        ...
        setDisplay: function(e) {
            ...
            } else if(elem.id == 'msgImgTf') {
                document.querySelectorAll('.campaign-popup').forEach(element => {
                    if(elem.checked) {
                        if(fileMsgImgRnm01.value == '' && document.querySelector('#msgImgNm01').value == '') {
                            element.setAttribute('style', '--campaign-profile-image: url(/assets/images/campaign/noimage_campaign_popup_b_c_01.png)');
                        } else {
                            const reader = new FileReader();
                            if(fileMsgImgRnm01.files && fileMsgImgRnm01.files[0]) {
                                reader.onload = function(e) {
                                    element.setAttribute('style', '--campaign-profile-image: url(' + e.target.result + ')');
                                };

                                reader.readAsDataURL(fileMsgImgRnm01.files[0]);
                            }
                        }
                    } else {
                        element.setAttribute('style', '--campaign-profile-image: ');
                    }
                });
            } else if(elem.id == 'msgButtonTf') {
                campaignMessageText.setPreviewButton();
        ...
        save: async function() {
            ...
            // 데이터 SET
            data.append('msgImgTf', (chkMsgImgTf.checked)? 'Y' : 'N');
            ...
            const pcHtml = campaignMessageText.setReplaceSrc(document.querySelector('#campaignMessagePreviewLaptop .campaign-popup').outerHTML);
            const moHtml = campaignMessageText.setReplaceSrc(document.querySelector('#campaignMessagePreviewMobile .campaign-popup').outerHTML);
        ...
        showHisMessage: function() {
            ...
            Popup.open(`/campaign/popup/historyMessage?acl=${acl}&msgType=${msgType}`, 'contentPopupMessageText');
        },
        ...
        setReplaceSrc: function(htmlStr) {
            ...
            tmpHtml.querySelector('.campaign-popup').removeAttribute('style');
