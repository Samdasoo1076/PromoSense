const campaignMessageVertical = (() => {
    ...
    const chkMsgTitleTf = document.querySelector('#msgTitleTf');                // 메인 문구 사용 여부
    const chkMsgContentsTf = document.querySelector('#msgContentsTf');          // 서브 문구 사용 여부
    ...
    return {
        init: function() {
            ...
            txtMsgButton.addEventListener('blur', this.setMsgButton);
            ...
            chkMsgTitleTf.addEventListener('click', this.setDisplay);
            chkMsgContentsTf.addEventListener('click', this.setDisplay);
            ...
            this.setDisplay(chkMsgTitleTf);
            this.setDisplay(chkMsgContentsTf);
        ...
        setMsgContents: function(e) {
            ...
            const view = element.querySelector('.board-summary');
                if(elem.id == 'msgContents') {
                    campaignMessageVertical.setContentsRow(e);
        ...
        setDisplay: function(e) {
            ...
            } else if(elem.id == 'msgButtonTf') { // 0328수정
                campaignMessageVertical.setPreviewButton();
            } else if(elem.id == 'msgLeaveTf') { // 0328수정
                campaignMessageVertical.setPreviewButton();
            } else if(elem.id == 'msgRegistTf') { // 0328수정
                campaignMessageVertical.setPreviewButton();
            } else if(elem.id == 'msgMobileCallingTf') { // 0328수정
                campaignMessageVertical.setPreviewButton();
            }
        },
        save: async function() {
            ...
            data.append('msgTitleTf', (chkMsgTitleTf.checked)? 'Y' : 'N');
            if(chkMsgTitleTf.checked) {
                data.append('msgTitle', txtMsgTitle.value);
                data.append('msgTitleColor', txtMsgTitleColor.value);
            }
            data.append('msgContentsTf', (chkMsgContentsTf.checked)? 'Y' : 'N');
            if(chkMsgContentsTf.checked) {
            ...
            const pcHtml = campaignMessageVertical.setReplaceSrc(document.querySelector('#campaignMessagePreviewLaptop .campaign-popup').outerHTML);
            const moHtml = campaignMessageVertical.setReplaceSrc(document.querySelector('#campaignMessagePreviewMobile .campaign-popup').outerHTML);
        ...
        showHisMessage: function() {
            ...
            Popup.open(`/campaign/popup/historyMessage?acl=${acl}&msgType=${msgType}`, 'contentPopupMessageVertical');
        },
        saveTemp: async function() {
            ...
            if(fileMsgImgRnm01.value != '') {
                ...
                if(constants.imageType.indexOf(fileType) == -1) {
                    await Modal.alert('형식에 맞지 않는 이미지입니다.');
                    return false;
                }
            }
