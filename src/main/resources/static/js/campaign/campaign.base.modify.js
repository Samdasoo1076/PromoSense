const campaignBaseModify = (() => {
    ...
    const divTg2ReturnView = document.querySelector('#tg2ReturnView');          // 2타겟 재방문 고개 선택시
    ...
    return {
        init: function() {
            ...
            radTgType2Fixeds.forEach(element => {
        ...
        viewCaPlace: function(e) {
            if(e.target.value == 'FIXED_PAGE') {
                ...
                campaignBaseModify.viewCaPlaceExcept();
            }
        },
        viewCaViewPoint: function(e) {
            if(e.target.value == 'PERCENT') {
                ...
                const tmpCaViewSub = document.querySelector('#tmpCaViewSub').value.toString();
                divCaViewPointPercent.style.display = 'none';
                divCaViewPointSecond.style.display = 'block';
                divCaViewPointOut.style.display = 'none';

                if(document.querySelector('#tmpCaView').value != e.target.value) {
                    document.querySelector('input[name=caViewSubSecond][value="1"]').checked = true;
                } else {
                    document.querySelector('input[name=caViewSubSecond][value="' + tmpCaViewSub + '"]').checked = true;
                }
        ...
        viewUrlAdd: function(e) {
            ...
            elButton.setAttribute('onclick', 'campaignBaseModify.viewUrlMinus(this);');
        ...
        makeDataAndVaild: function() {
            ...
            data['id'] = document.querySelector('#caId').value;
            data['caState'] = document.querySelector('#caState').value;
            data['visibleCnt'] = document.querySelector('#visibleCnt').value;
            data['clickCnt'] = document.querySelector('#clickCnt').value;
            data['closeCnt'] = document.querySelector('#closeCnt').value;
            data['phoneCnt'] = document.querySelector('#phoneCnt').value;
            data['targetVisitCnt'] = document.querySelector('#targetVisitCnt').value;
            data['leaveCnt'] = document.querySelector('#leaveCnt').value;
            data['registCnt'] = document.querySelector('#registCnt').value;
            data['visibleMoCnt'] = document.querySelector('#visibleMoCnt').value;
            data['clickMoCnt'] = document.querySelector('#clickMoCnt').value;
            data['closeMoCnt'] = document.querySelector('#closeMoCnt').value;
            data['targetVisitMoCnt'] = document.querySelector('#targetVisitMoCnt').value;
            data['leaveMoCnt'] = document.querySelector('#leaveMoCnt').value;
            data['registMoCnt'] = document.querySelector('#registMoCnt').value;
            data['useTf'] = document.querySelector('#useTf').value;
            data['delTf'] = document.querySelector('#delTf').value;
            data['viewOrder'] = document.querySelector('#viewOrder').value;

            return data;
        },
        save: async function() {
            const data = campaignBaseModify.makeDataAndVaild();
            if(!data) return;
            const acl = document.querySelector('#acl').value;

            if(document.querySelector('#messageType').value != ''
                && document.querySelector('#messageType').value != document.querySelector('input[name=caMsgType]:checked').value) {
                await Modal.confirm('메시지 유형 변경 시 기존에 작성한 2단계(Create Message) \n내용은 모두 초기화됩니다. 그래도 변경하시겠습니까?',
                    () =>{
                        domsFetch(`/campaign/base/modify.do?acl=${acl}`, {
                            method: 'POST',
                            headers: {
